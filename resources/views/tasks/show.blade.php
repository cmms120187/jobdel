                ...
                <div class="lg:col-span-2">
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                        <div class="p-6">
                            ...
                            <div class="mb-4">
                                <span class="px-3 py-1 text-sm rounded {{ $task->status === 'completed' ? 'bg-green-100 text-green-800' : ($task->status === 'in_progress' ? 'bg-blue-100 text-blue-800' : 'bg-yellow-100 text-yellow-800') }}">
                                    {{ ucfirst($task->status) }}
                                </span>
                                <span class="px-3 py-1 text-sm rounded ml-2 {{ $task->priority === 'high' ? 'bg-red-100 text-red-800' : ($task->priority === 'medium' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-800') }}">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm mb-4">
                                @if($task->room)
                                    <div>
                                        ...
                                        <p class="text-gray-600">
                                            <a href="{{ route('rooms.show', $task->room) }}" class="text-blue-600 hover:text-blue-800 font-semibold">
                                                {{ $task->room->room }}
                                            </a>
                                ...
                                @if($task->project_code)
                                    <div>
                                        ...
                                        <p class="text-gray-600">{{ $task->project_code }}</p>
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-4 text-sm">
                                ...
                                @if($task->add_request)
                                    <div>
                                        ...
                                        <p class="text-gray-600">{{ $task->add_request }}</p>
                                    </div>
                                @endif
                                @if($task->start_date)
                                    <div>
                                        ...
                                        <div class="mt-2">
                                            <div class="flex items-center gap-2">
                                                <div class="w-48 bg-gray-200 rounded-full h-2.5">
                                                    <div class="bg-gradient-to-r from-blue-600 to-indigo-600 h-2.5 rounded-full transition-all" style="width: {{ $overallProgress }}%"></div>
                                                </div>
                                                <span class="text-sm font-semibold text-gray-700">{{ $overallProgress }}%</span>
                                            </div>
                                    ...
                                    <div class="space-y-4">
                                        ...
                                        <div class="grid grid-cols-2 gap-4 mt-4">
                                            <div>
                                                ...
                                                <input type="date" name="start_date" id="start_date" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                            </div>
                                            <div>
                                                <label for="start_time" class="block text-sm font-medium text-gray-700 mb-1">Start Time</label>
                                                <input type="time" name="start_time" id="start_time" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                            </div>
                                        </div>
                                        <div class="grid grid-cols-2 gap-4 mt-4">
                                            <div>
                                            ...
                                            <div>
                                                ...
                                                <input type="time" name="due_time" id="due_time" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                        ...
                                        <div class="border rounded-lg p-4 hover:shadow-md transition-shadow">
                                            <div class="flex justify-between items-start mb-3">
                                                <div class="flex-1">
                                                    <div class="flex items-center gap-2 mb-1">
                                                        ...
                                                        <span class="px-2 py-1 text-xs rounded {{ $item->status === 'completed' ? 'bg-green-100 text-green-800' : ($item->status === 'in_progress' ? 'bg-blue-100 text-blue-800' : 'bg-yellow-100 text-yellow-800') }}">
                                                    ...
                                                    <div class="flex items-center gap-4 text-xs text-gray-500">
                                                        ...
                                                        @endif
                                                        @if($item->start_date)
                                                            <span>Start: {{ $item->start_date->format('d M Y') }}{{ $item->start_time ? ' ' . $item->start_time : '' }}</span>
                                                        @endif
                                                        @if($item->due_date)
                                                            <span class="{{ $item->isOverdue() ? 'text-red-600 font-bold' : '' }}">
                                                ...
                                                <div class="flex gap-2">
                                                    ...
                                                    @endphp
                                                    @if($isSuperuser)
                                                        <a href="{{ route('task-items.edit', [$task, $item]) }}" class="bg-indigo-500 hover:bg-indigo-700 text-white text-xs font-bold py-1.5 px-3 rounded inline-flex items-center">
                                                    ...
                                                    @if($canUpdateProgress)
                                                        @if($item->progress_percentage < 100)
                                                            <button onclick="openProgressModal({{ $item->id }}, {{ $item->progress_percentage }}, '{{ $item->title }}')" class="bg-blue-500 hover:bg-blue-700 text-white text-xs font-bold py-1.5 px-3 rounded">
                                                                Update Progress
                                                            </button>
                                                        @else
                                                            <button onclick="openProgressModal({{ $item->id }}, {{ $item->progress_percentage }}, '{{ $item->title }}')" class="bg-green-500 hover:bg-green-700 text-white text-xs font-bold py-1.5 px-3 rounded" title="Progress sudah 100%. Bisa tambah catatan/foto">
                                                                Tambah Catatan/Foto
                                                            </button>
                                                        @endif
                                                    @endif
                                                    <form action="{{ route('task-items.destroy', [$task, $item]) }}" method="POST" onsubmit="return confirm('Hapus detail pekerjaan ini?');" class="inline">
                                                        @csrf
                                                        @method('DELETE')
                                            ...
                                            <div class="mb-3">
                                                <div class="w-full bg-gray-200 rounded-full h-3">
                                                    <div class="bg-gradient-to-r from-blue-600 to-indigo-600 h-3 rounded-full transition-all flex items-center justify-end pr-2" style="width: {{ $item->progress_percentage }}%">
                                                        @if($item->progress_percentage > 15)
                                                            <span class="text-xs font-bold text-white">{{ $item->progress_percentage }}%</span>
                                                        @endif
                                                    </div>
                                                </div>
                                                @if($item->progress_percentage <= 15)
                                                    <p class="text-xs text-gray-600 mt-1">Progress: {{ $item->progress_percentage }}%</p>
                                                @endif
                                            </div>
                                                    ...
                                                    <div class="space-y-2 max-h-60 overflow-y-auto">
                                                        @foreach($item->updates->take(5) as $update)
                                                            <div class="bg-gray-50 rounded p-3 border-l-2 border-blue-500">
                                                                    ...
                                                                    <div class="flex-1">
                                                                        <span class="text-xs font-bold text-blue-600">
                                                                            @if($update->new_progress_percentage !== null)
                                                                                Progress: {{ $update->old_progress_percentage ?? 0 }}% → {{ $update->new_progress_percentage }}%
                                                                            @endif
                                                                            @if($update->new_status)
                                                                                | Status: {{ ucfirst($update->old_status ?? 'pending') }} → {{ ucfirst($update->new_status) }}
                                                                            @endif
                                                                    </span>
                                                                    <div class="flex items-center gap-2 mt-1 flex-wrap">
                                                                        <span class="text-xs text-gray-500">
                                                                            📅 {{ $update->update_date ? $update->update_date->format('d M Y') : $update->created_at->format('d M Y') }}
                                                                        </span>
                                                                        ...
                                                                        @else
                                                                            <span class="text-xs text-gray-400">|</span>
                                                                            <span class="text-xs text-gray-500">{{ $update->created_at->format('H:i') }}</span>
                                                                        @endif
                                                                    </div>
                                                                </div>
                                                                    @if(Auth::user()->position && Auth::user()->position->name === 'Superuser')
                                                                        <a href="{{ route('task-items.update.edit', [$task, $item, $update]) }}" class="text-xs text-indigo-600 hover:text-indigo-800 ml-2" title="Edit Update">
                                                                ...
                                                                @if($update->notes)
                                                                    <p class="text-xs text-gray-700 mt-1">{{ $update->notes }}</p>
                                                                @endif
